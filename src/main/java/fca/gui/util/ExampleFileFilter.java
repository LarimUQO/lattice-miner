...
/*
 * @(#)ExampleFileFilter.java 1.9 99/04/23 Copyright (c) 1998, 1999 by Sun Microsystems, Inc. All
 * Rights Reserved. Sun grants you ("Licensee") a non-exclusive, royalty free, license to use,
 * modify and redistribute this software in source and binary code form, provided that i) this
 * copyright notice and license appear on all copies of the software; and ii) Licensee does not
 * utilize the software in a manner which is disparaging to Sun. This software is provided "AS IS,"
 ...
 * DAMAGES SUFFERED BY LICENSEE AS A RESULT OF USING, MODIFYING OR DISTRIBUTING THE SOFTWARE OR ITS
 ...
 * SOFTWARE, EVEN IF SUN HAS BEEN ADVISED OF THE POSSIBILITY OF SUCH DAMAGES. This software is not
 * designed or intended for use in on-line control of aircraft, air traffic, aircraft navigation or
 * aircraft communications; or in the design, construction, operation or maintenance of any nuclear
 * facility. Licensee represents and warrants that it will not use or redistribute the Software for
 * such purposes.
 */
...
public class ExampleFileFilter extends FileFilter {
	 ...
	 * "JPEG Image Images"); Note that the "." before the extension is not needed. If provided, it
	 * will be ignored.
	 * @see #addExtension
	 */
	 ...
	 * ExampleFileFilter(String {"gif", "jpg"}, "Gif and JPG Images"); Note that the "." before the
	...
	/**
	 * Return the extension for the filter
	 */
	...
	/**
	 * Return the extension for the filter by a list
	 */
	public ArrayList<String> getExtensionsList() {
		if (filters != null) {
			Enumeration<String> extensions = filters.keys();
			ArrayList<String> res = new ArrayList<String>();
			res.add(extensions.nextElement());
	 ...
	 * filter.addExtension("tif"); Note that the "." before the extension is not needed and will be
	 * ignored.
	 */
	...
	public String getDescription() {
		if (fullDescription == null) {
			if (description == null || isExtensionListInDescription()) {
				...
				Enumeration<String> extensions = filters.keys();
				if (extensions != null) {
					fullDescription += "." + extensions.nextElement(); //$NON-NLS-1$
					while (extensions.hasMoreElements()) {
						fullDescription += ", ." + extensions.nextElement(); //$NON-NLS-1$
					}
				}
