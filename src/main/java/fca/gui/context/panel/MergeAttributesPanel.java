...
/**
 * Panneau affichant les attributs d'un contexte pour en permettre la fusion
 * @author Geneviève Roberge
 * @version 1.0
 */
public class MergeAttributesPanel extends JFrame implements ActionListener {
	
	/**
	 * 
	 */
	private static final long serialVersionUID = -3689898239207195645L;
	...
	JButton mergeBtn; //Bouton pour exécuter la projection
	 ...
	 */
	public MergeAttributesPanel(ContextTableScrollPane ctsp, ContextViewer cv) {
		//setDefaultLookAndFeelDecorated(true);
		setDefaultCloseOperation(WindowConstants.HIDE_ON_CLOSE);
		setTitle(GUIMessages.getString("GUI.mergeAttributes")); //$NON-NLS-1$
		...
		TitledBorder panelTitle = new TitledBorder(BorderFactory.createLineBorder(Color.BLACK),
				GUIMessages.getString("GUI.selectAttributesToMerge")+" : "); //$NON-NLS-1$ //$NON-NLS-2$
		panelTitle.setTitleColor(Color.BLACK);
		panel.setBorder(panelTitle);
		
		mergeBtn = new JButton(LMIcons.getOK());
		mergeBtn.addActionListener(this);
		mergeBtn.setToolTipText(GUIMessages.getString("GUI.executeAndQuit")); //$NON-NLS-1$
		...
		if (e.getSource() == mergeBtn) {
			...
			for (int i = 0; i < attributeCheckBoxes.size(); i++) {
				...
				if (checkBox.isSelected()) {
					intent.add(checkBox.getText());
					newName = newName + checkBox.getText() + "V"; //$NON-NLS-1$
				}
			}
			newName = newName.substring(0, newName.length() - 1);
			
			try {
				context.mergeAttributes(intent, newName);
