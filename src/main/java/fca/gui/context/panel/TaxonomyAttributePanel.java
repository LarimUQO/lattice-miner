...
/**
 * Panneau affichant les attributs d'un contexte et certains opérateurs logiques
 * pour creer une taxonomie definissant un nouvel attribut
 * @author Linda Bogni
 * @version 1.0
 */
public class TaxonomyAttributePanel extends JFrame implements ActionListener{
	...
	Vector<String> oldAttributes; //attributs a retirer du contexte apres taxonomie	
	 ...
	 */
	public TaxonomyAttributePanel(ContextTableScrollPane ctsp, ContextViewer cv){
		//setDefaultLookAndFeelDecorated(true);
		setDefaultCloseOperation(WindowConstants.HIDE_ON_CLOSE);
		setTitle(GUIMessages.getString("GUI.attributeGeneralisation")); //$NON-NLS-1$
		...
		/*initialisation du vector contenant les attributs a retirer du contexte*/ //-->LINDA
		oldAttributes=new Vector<String>();
		...
		for (int i = 0; i < attributes.size(); i++) {
			...
			//String fa1=fa.substring(0,fa.indexOf(" : "));//Ne prend que l'attribut cible-->LINDA
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == createBtn) {
			...
			if (!formula.isValid()) {
				...
				try {//-----ICI
					context.createTaxonomyAttribute(formula, expression);
			...
			oldAttributes.add(button.getText()); //Sauver les boutons selectionnes-->LINDA
			if (expressionField.getCaretPosition() < expressionField.getText().length()) {
				String text1 = expressionField.getText().substring(0, expressionField.getCaretPosition());
				String text2 = expressionField.getText().substring(expressionField.getCaretPosition());
